...
def published_listing(db, owner_user, owner_property):
    listing = Listing.objects.create(
        ...
        title="Published Transaction Listing",
        description="A listing ready for transaction tests.",
        price="10000000.00",
        currency="TZS",
...
def accepted_offer(db, requester_user, owner_user, owner_property):
    listing = Listing.objects.create(
        property=owner_property,
        title="Accepted Offer Listing",
        description="A listing with an accepted offer.",
        price="10000000.00",
        currency="TZS",
        status=ListingStatus.PUBLISHED,
    )
